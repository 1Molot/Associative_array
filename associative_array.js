...
const newArr = [];
for (const movie in movies) {
    if (movies[movie].director === "Christopher Nolan") {
        newArr.push(movie);
    }
}
console.log(newArr)
...
// const reduceGames = games.reduce((acc,el)=>{
//     acc[el.title] = el;
//     delete el.title;
//     return acc;
// },{})
...
const directors = Object.value(movies).map(el => el.director);
...
let films90 = []
for (const movie in movies) {
    if (movies[movie].year <= 1999 && movies[movie].year >= 1990) {
        films90.push(movie)
    }
}
console.log(films90)
...
for (const movie in movies) {
    filmSort.push([movie, movies[movie].year])
}
filmSort.sort((a,b) => a[1]-b[1])
...
const swapObjectKeysAndValues = (obje) => {
    const swapped = {};
//     for (const key in obj) {
//         swapped[obj[key]] = key;
//         console.log(swapped[obje[key]])
// }
//     return swapped
...
const swappedObj = swapObjectKeysAndValues(obje);
